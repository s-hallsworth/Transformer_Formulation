...
import transformer_b_exp_dx as TNN
...
def TNN(i, tnn_input, std, tps, seq_len, model):
    transformer.add_layer_norm(model,tnn_input, "layer_norm_"+str(i), "gamma1", "beta1",std)
    transformer.add_attention(model, "layer_norm_"+str(i),"attention_output_"+str(i), tps.W_q, tps.W_k, tps.W_v, tps.W_o, tps.b_q, tps.b_k, tps.b_v, tps.b_o)
    transformer.add_residual_connection(model,"input_embed_"+str(i), "attention_output_"+str(i), "residual_1_"+str(i))
    transformer.add_layer_norm(model, "residual_1_"+str(i), "layer_norm_2_"+str(i), "gamma2", "beta2")
    nn, input_nn, output_nn = transformer.get_fnn(model, "layer_norm_2_"+str(i), "FFN_1_"+str(i), "ffn_1", (seq_len,2), tps.parameters)
    transformer.add_residual_connection(model,"residual_1_"+str(i), "FFN_1_"+str(i), "residual_2_"+str(i))  
    transformer.add_avg_pool(model, "residual_2_"+str(i), "avg_pool_"+str(i))
    nn2, input_nn2, output_nn2 = transformer.get_fnn(model, "avg_pool_"+str(i), "FFN_2_"+str(i), "ffn_2", (1,2), tps.parameters)
    
    return [nn, input_nn, output_nn], [nn2, input_nn2, output_nn2]



model = tps.setup_toy( T, 0 ,seq_len, pred_len, model_path, config_file)
...
for t in range(1, T+1):
    tnn_input = model.X[model.time.at(t): model.time.at(t+seq_len) ,:]
    out1, out2 = TNN(t, tnn_input, std, tps, seq_len, model)
