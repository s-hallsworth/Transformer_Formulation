...
Pred_window = 2
std = 0.82
transformer.embed_input(model, "input_param", "input_embed", W_emb = tps.W_emb, b_emb=tps.b_emb)
...
nn, input_nn, output_nn = transformer.get_fnn(model, "layer_norm_2", "FFN_1", "ffn_2", (seq_len,2), tps.parameters)
transformer.add_residual_connection(model,"residual_1", "FFN_1", "residual_2")  
transformer.add_avg_pool(model, "residual_2", "avg_pool")
nn2, input_nn2, output_nn2 = transformer.get_fnn(model, "avg_pool", "FFN_2", "ffn_3", (1,2), tps.parameters)

     
# Define transformer 2

  
transformer2.embed_input(model, "input_2","input_embed2", W_emb = tps.W_emb, b_emb=tps.b_emb)
...
nn21, input_nn21, output_nn21 = transformer2.get_fnn(model, "layer_norm_22", "FFN_12", "ffn_2", (seq_len,2), tps.parameters)
...
nn22, input_nn22, output_nn22 = transformer2.get_fnn(model, "avg_pool22", "FFN_22", "ffn_3", (1,2), tps.parameters)
...
last_time_1 = False
last_time_2 = False
d = model.FFN_2.index_set().first()

    
# for t_index, t in enumerate(model.time):
#     if t > model.time_input.first() and t < model.time_input.last():
#         model.input_var_constraints.add(expr=model.input_2[t] == model.input_var[t,d])
