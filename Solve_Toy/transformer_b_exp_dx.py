...
class Transformer:
    ...
    def embed_input(self, M, input_var_name, embed_var_name, W_emb=None, b_emb = None):
        ...
        if input_var.is_indexed():
            ...
            if not hasattr(M, embed_var_name):
                setattr(M, embed_var_name, pyo.Var(indices[0], M.model_dims, within=pyo.Reals, initialize= 0))
            ...
            if W_emb is None:
                ...
                if not b_emb is None:
                    ...
                    setattr(M, embed_var_name+"_b_emb", pyo.Param(M.model_dims, initialize=b_emb_dict))
                    b_emb= getattr(M, embed_var_name+"_b_emb")  
    ...
    def add_layer_norm(self, M, input_var, layer_norm_var_name, gamma= None, beta = None, std=None):  # non-linear
        """
    ...
    def add_attention(self, M, input_var_name, output_var_name, W_q, W_k, W_v, W_o, b_q = None, b_k = None, b_v = None, b_o = None):
        ...
        for h in MHA_Block.heads:
            ...
            for n in self.time_input:
                ...
                MHA_Block.compatibility_exp_sum[h, n].ub = sum( MHA_Block.compatibility_exp[h,n,p].ub for p in self.time_input) # exp --> pos
