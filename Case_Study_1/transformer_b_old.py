...
import omlt
from helpers import GUROBI_ML_helper, OMLT_helper 
...
class Transformer:
    ...
    def __build_layers(self, layer_names, parameters, enc_input, dec_input, enc_bounds = None, dec_bounds=None):
        ...
        for l, layer in enumerate(layer_names):
            ...
            if "layer_norm" in layer:
                ...
                self.add_layer_norm( input_name, layer, f"{layer}_gamma", f"{layer}_beta")
    ...
    def add_input_var(self, input_var_name, dims=(2,2), bounds=(None, None)):
    ...
    def add_layer_norm(self, input_var_name, layer_norm_var_name, gamma= None, beta = None, std=None):  # non-linear
        ...
        if not hasattr( self.M, layer_norm_var_name):
            ...
            if not gamma is None and not beta is None:
                ...
                dict_gamma = {(v): val for v,val in zip( model_dims, gamma)}
                dict_beta  = {(v): val for v,val in zip( model_dims, beta)}
            else:
                dict_gamma = {(v): 0 for v in model_dims}
                dict_beta  = {(v): 0 for v in model_dims}
                
            # define new gamma and beta params
            setattr( self.M, f"gamma_{layer_norm_var_name}", pyo.Param( model_dims, initialize = dict_gamma))
            setattr( self.M, f"beta_{layer_norm_var_name}", pyo.Param( model_dims, initialize = dict_beta))
