...
class Transformer:
    ...
    def add_layer_norm(self, input_var_name, layer_norm_var_name, gamma= None, beta = None, std=None, epsilon=1e-6):  # non-linear
        ...
        if not hasattr( self.M, layer_norm_var_name):
            ...
            sign_name = 'sign_'+ layer_norm_var_name
            setattr( self.M, sign_name, pyo.Var(time_dim, within=pyo.Reals))
            sign = getattr( self.M, sign_name)
            sign2_name = 'sign2_'+ layer_norm_var_name
            setattr( self.M, sign2_name, pyo.Var(time_dim, within=pyo.Reals))
            sign2 = getattr( self.M, sign2_name)
        ...
        for t in time_dim: 
            ...
            # self.M.layer_norm_constraints.add(expr= (sign[t]**2) == sign2[t])
            # self.M.layer_norm_constraints.add(expr= sign[t] == sign2[t] * sign[t])
            ...
            for d in model_dims:  
                ...
                if input_var[t, d].ub and input_var[t, d].lb:
                    ...
                    if not std :
                        denominator[t].ub =  max(input_var[t,:].ub) - min(input_var[t,:].lb) #standard deviation
                        denominator[t].lb = - ( max(input_var[t,:].ub) - min(input_var[t,:].lb))#/8
        ...
        for h in MHA_Block.heads:
            ...
            for n in time_dim:
                    ...
                    for p in res_dim_1_kv:
                        ...
                        # from Keras Softmax: 
                        # ``` python
                        # exp_x = exp(x - max(x)) #subtract max to avoid numeric overflow
                        # f(x) = exp_x / sum(exp_x)
                        # ```
                    ...
                    MHA_Block.attention_constraints.add(expr= MHA_Block.compatibility_exp_sum[h, n] == sum(MHA_Block.compatibility_exp[h, n, p] for p in res_dim_1_kv) + self.epsilon) 
