...
class TestTransformer(unittest.TestCase):    
    ...
    #     transformer.add_input_var("input_embed", dims=(seq_len, transformer.input_dim), bounds=(-3,3))
    #     transformer.add_layer_norm( "input_embed", "layer_norm", gamma1, beta1)
    #     transformer.add_attention( "layer_norm","attention_output", W_q, W_k, W_v, W_o, b_q, b_k, b_v, b_o, tnn_from="keras")
    ...
    #     print("- LN1 formulation == LN1 input model")
    ...
    #     self.assertIsNone(np.testing.assert_array_equal(actual.shape, expected.shape)) # compare shape with transformer
    #     self.assertIsNone(np.testing.assert_array_almost_equal(actual, expected, decimal=2)) # compare value with transformer output
    ...
    #     transformer.add_input_var("input_embed", dims=(seq_len, transformer.input_dim), bounds=(-3,3))
    #     transformer.add_layer_norm( "input_embed", "layer_norm", gamma1, beta1)
    #     transformer.add_attention( "layer_norm","attention_output", W_q, W_k, W_v, W_o, b_q, b_k, b_v, b_o, exp_approx=True) # --- #
    #     transformer.add_residual_connection("input_embed", "attention_output", "residual_1")
    #     transformer.add_layer_norm( "residual_1", "layer_norm_2", gamma2, beta2)
    ...
    def test_layers(self):
        ...
        transformer.add_input_var("input_embed", dims=(seq_len, transformer.input_dim), bounds=(-3,3))
        transformer.add_layer_norm( "input_embed", "layer_norm", gamma1, beta1)
        transformer.add_attention( "layer_norm","attention_output", W_q, W_k, W_v, W_o, b_q, b_k, b_v, b_o, exp_approx=True)#, tnn_from='keras')
        transformer.add_residual_connection("input_embed", "attention_output", "residual_1")
        transformer.add_layer_norm( "residual_1", "layer_norm_2", gamma2, beta2)
        
        output_var = transformer.add_FFN_2D("layer_norm_2", "ffn_1", "ffn_1", (seq_len, 2), parameters, bounds = None)
        output_var = transformer.add_residual_connection("residual_1", output_var, "residual_2") 
        output_nn2 = transformer.add_FFN_2D(output_var, "ffn_2", "ffn_2", (seq_len, 2), parameters, bounds = None)
        #nn, input_nn, output_nn = transformer.get_fnn("layer_norm_2", "ffn_1", "ffn_1", (seq_len,2), parameters)
        ...
        #--- for pyomo --#
        x1 = list(optimal_parameters['x1'].values())
        x2 = list(optimal_parameters['x2'].values())
        ...
        plt.plot(time, opt_x2, color='tab:green', label= "y expected opt. trajectory")
        plt.plot(time, x2, '--x', color='r', label = f'y opt. trajectory')

        plt.xlabel("time")
        plt.ylabel("height")
        plt.title(f'Optimal Trajectory y Solution with fixed TNN inputs')
        plt.legend()
        plt.show()
        ...
        plt.plot(time, opt_x1, color='tab:orange', label= "x expected opt. trajectory")
        
        plt.plot(time, x1, '--x', color='b', label = f'x opt. trajectory')
        plt.xlabel("time")
        plt.ylabel("distance")
        plt.title(f'Optimal Trajectory x Solution with fixed TNN inputs')
        plt.legend()
        plt.show()
        ...
        FFN_out = np.array(layer_outputs_dict["dense_4"])[0].transpose(1,0).transpose(1,0).flatten()
        
        print("Output: ", ffn_2_output)
        print("expected:", FFN_out)
        print( np.array([optimal_parameters["ffn_2"]]))
        print( np.array([optimal_parameters["x1"]])[0])
        print(input_x1)
        print( np.array([optimal_parameters["x2"]])[0])
        print(input_x2)
