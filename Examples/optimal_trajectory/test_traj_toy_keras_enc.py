...
"""
Unit testing suite for Transformer-based optimization models.

This script performs unit tests to verify the numerical accuracy and formulation consistency 
of integrating pretrained Transformer-based Neural Networks (TNNs) into an optimization framework 
using Pyomo and Gurobi. The Transformer model is tested for various components, including 
input embedding, layer normalization, multi-head attention, feed-forward networks (FFNs), 
and add-and-norm layers.

Key Functionalities:
1. Validate the integration of TNN components into optimization models.
2. Ensure the correctness of input, intermediate, and output calculations against pretrained weights.
3. Test and debug specific Transformer layers like attention and feed-forward layers.
4. Visualize optimal trajectories and predictions for trajectory optimization problems.

Main Classes and Functions:
1. `TestTransformer`:
   - **test_input**: Validates the input embedding layer of the Transformer.
   - **test_LN1**: Tests the first layer normalization module.
   - **test_MHA**: Checks the multi-head attention module for correctness.
   - **test_ADDNORM1**: Tests the Add-and-Norm operation combining attention output with residuals.
   - **test_FFN1**: Validates the feed-forward network layer after attention.
   - **test_layers**: Tests the full integration of Transformer layers in sequence.

2. Helper Functions:
   - `get_optimal_dict(result, model)`: Extracts optimal parameter values from a Pyomo optimization model.
   - `reformat(dict, layer_name)`: Reshapes Pyomo variable outputs to match Transformer tensor formats.

3. Optimization Problem Setup:
   - A toy trajectory optimization problem is defined where the trajectory of a projectile 
     is modeled using TNN-based constraints. The objective is to minimize the deviation from 
     the target trajectory.

4. Integration with Gurobi:
   - Converts Pyomo models to Gurobi format and adds FFN constraints using `gurobi_ml`.

5. Visualization:
   - Generates plots to compare:
     - Target trajectories.
     - Predicted trajectories from the optimization model.
     - Expected optimal trajectories.

Usage:
1. Place pretrained Transformer models and configuration files in the appropriate paths.
2. Run the script using: python test_traj_toy_keras_enc.py
3. Observe terminal outputs for assertion results and ensure all tests pass.
4. Visualize trajectory plots for additional insights.
...
class TestTransformer(unittest.TestCase):    
    ...
    def test_MHA(self):
        ...
        self.assertIsNone(np.testing.assert_array_almost_equal(computed_attn_output.flatten(), expected, decimal =5))
        self.assertIsNone(np.testing.assert_array_equal(attn_scores.flatten().shape, attn_score_form.shape))
        self.assertIsNone(np.testing.assert_array_almost_equal( attn_score_form, attn_scores.flatten(), decimal =5))
        ...
        self.assertIsNone(np.testing.assert_array_almost_equal( attn_weight_form, attn_weights.flatten(), decimal =5))
...
if __name__ == '__main__': 
    ...
    hyper_params = '.\\data\\toy_traj_k_enc_config_2.json' 
    ...
    model_path = ".\\training\\models\\TNN_traj_enc_2.keras" # dmodel 4, num heads 1, n ence 1, n dec 1, head dim 4, pred_len 2+1 
    ...
    #         bound_cut[elem] = val["act_val"] # set activation dict to new combi
    #         bound_cut["MHA_softmax_env"] = False
