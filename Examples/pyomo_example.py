...
model.conc = pyo.Var(model.components, within=pyo.NonNegativeReals) # create indexed variable with concentration of every component
model.F = pyo.Var(within=pyo.NonNegativeReals)

# specifying constraints
model.mass_bal_A = pyo.Constraint(expr= 
                                  model.F * (model.conc['Ain']-model.conc['A']) 
                                  - (model.k['1']*model.conc['A']*model.vol)
                                  == 0
                                  )
def mass_bal_B_rule(m):
    return((-m.F *m.conc['B']) + (m.k['1']*m.conc['A']-m.k['2']*m.conc['B'])*model.vol == 0) # equations can be indexed too

model.mass_bal_B = pyo.Constraint(rule=mass_bal_B_rule)

# define objective function
model.obj = pyo.Objective(expr=model.conc['B'], sense=-1) # -1: maximize, +1: minimize (default)

#specify bounds
model.conc['Ain'].setub(1) #lower bound set by nonnegreal
model.conc['A'].setub(0.1) #on indexed set can set all of the bounds when declaering if bounds are the same
model.F.setub(20)
