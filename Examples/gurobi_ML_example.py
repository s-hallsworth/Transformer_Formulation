...
import numpy as np
from sklearn.datasets import make_regression #to generate random data for the regression
...
nn = MLPRegressor(hidden_layer_sizes=[20] * 2, activation='relu', solver='adam', max_iter=10000, random_state=1) #max_iter: # of epochs
nn.fit(X, y)
...
input_vars = m.addMVar(X_examples.shape, lb=X_examples-delta, ub=X_examples+0.2) # add matrix variable with same shape as X_examples
output_vars = m.addMVar(y_examples.shape, lb=-gp.GRB.INFINITY) # set -inf lb since Gurobi vars are non-negative

## Add constraint to link NN input_vars and output_vars
pred_constr = add_predictor_constr(m, nn, input_vars, output_vars) # modeling object returned. *NB* due to shape of vars, n different constraints will be added
...
print()
print('Error:',pred_constr.get_error()) #computes error g(X_example)-y using original regression nn. 
...
print()
print("objective value:",m.getObjective().getValue()) # output var^2. why only 1 value?
