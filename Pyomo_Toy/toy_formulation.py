...
class transformer:
    ...
    def add_attention(self, M):
        ...
        W_q_dict = {(h, m, k): self.W_q[h-1][m-1][k-1] for h in M.heads for m in M.model_dims for k in M.k_dims}
        W_k_dict = {(h, m, k): self.W_k[h-1][m-1][k-1] for h in M.heads for m in M.model_dims for k in M.k_dims}
        W_v_dict = {(h, m, k): self.W_v[h-1][m-1][k-1] for h in M.heads for m in M.model_dims for k in M.k_dims}
        W_o_dict = {(h, m, k): self.W_o[h-1][m-1][k-1] for h in M.heads for m in M.model_dims for k in M.k_dims}
        ...
        M.attention_score = pyo.Var(M.heads, M.time, M.k_dims) # softmax ( sqrt(Q * K) ) * V
        M.attention_output = pyo.Var(M.time, M.model_dims) # concat heads and linear transform
        
        for h in M.heads:
            for n in M.time:
                for k in M.k_dims:
                    # constraints for Query, Key and Value
                    M.attention_constraints.add(expr = M.Q[h,n,k] == sum(M.x_embed[n, m] * M.W_q[h, m, k] for m in M.model_dims ))
                    M.attention_constraints.add(expr = M.K[h,n,k] == sum(M.x_embed[n, m] * M.W_k[h, m, k] for m in M.model_dims ))
                    M.attention_constraints.add(expr = M.V[h,n,k] == sum(M.x_embed[n, m] * M.W_v[h, m, k] for m in M.model_dims ))
        
                    # attention score = sum(attention_weight * V)
                    M.attention_constraints.add(expr = M.attention_score[h,n,k]  == sum( M.attention_weight[h,n,n2] * M.V[h,n2,k] for n2 in M.time))
                                                
                for n2 in M.time:
                    # compatibility sqrt(Q * K) across all pairs of elements 
                    M.attention_constraints.add(expr = M.compatability[h,n,n2] == sum(M.Q[h, n, k] * M.K[h, n, k] for k in M.k_dims )**(1/self.d_k)) # non-linear

                    # attention weights softmax(compatibility)
                    M.attention_constraints.add(expr = M.attention_weight[h,n,n2] == pyo.exp(M.compatability[h,n,n2]) / sum(pyo.exp(M.compatability[h,n,p]) for p in M.time))
        
              
        # output constraint
        for n in M.time:
            for m in M.model_dims:
                M.attention_constraints.add(expr = M.attention_output[n,m] == sum( sum( M.attention_score[h,n,k] * M.W_o[h,m,k] for k in M.k_dims) for h in M.heads) )
    ...
    def add_output_constraints(self, M): 
        ...
        for t in M.time:            
            if t > 0.9:   
                # predict x, u 
                transformer_output = predictions = [0, 0]
